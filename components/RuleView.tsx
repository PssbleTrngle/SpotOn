...
const RuleView: FC<IBaseRule & {
...
}> = ({ type, children, display, onAdd, onSelect, value, id = 'create', selected, errors = [], level = 0 }) => {

   const tooltip = useTooltip(`rule-${id}`)

   const messages = useMemo(() => errors
   ...
   const nextErrors = useCallback((index: number) => errors
      .map(e => ({ ...e, path: nextPath(index, e.path) }))
      .filter(e => e.path) as RuleError[],
      [errors, nextPath]
   )

   return (
      <Bubble data-tip={messages} data-for={`rule-${id}`} level={level} selected={selected?.length === 0} error={!!messages} onClick={click}>

         {level === 0 && tooltip}

         {!children && ((display || value)
            ? <span>{display ?? (value as any).toString()}</span>
            : <Type>{type}</Type>
         )}
         ...
         {children?.map((child, i) =>
            <Fragment key={child.id ?? i}>
               ...
               <RuleView
                  {...child}
                  level={level + 1}
                  selected={nextPath(i, selected)}
                  errors={nextErrors(i)}
                  onSelect={p => onSelect?.([i, ...p])}
                  onAdd={p => onAdd?.([i, ...p])}
               />
         ...
         {children && onAdd && <Add onClick={add}>+</Add>}
